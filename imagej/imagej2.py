    ...
    class ModuleService(object):
        ...
        def getModules(self):
            ...
            module_iterator = J.call(modules, "iterator",
                                     "()Ljava/util/Iterator;")
            return [wrap_module_info(x) for x in
                    J.iterate_java(module_iterator)]
        ...
        def getIndex(self):
            ...
            index.getC = lambda c: J.get_collection_wrapper(
                J.call(index.o, "get", "(Ljava/lang/Class;)Ljava/util/List;",
                       c),
                wrap_module_info)
...
def create_dataset(context, pixel_data, name=None, axes=None):
    ...
    dataset = dataset_service.create1(
        numpy.array(pixel_data.shape), name, axes, 64, True, True)
    imgplus = dataset.getImgPlus()
    #
    # Now use a copying utility to fill the imgplus with array data
    #
    strides = numpy.cumprod([1] + list(pixel_data.shape[:0:-1]))[::-1]
...
def create_overlay(context, mask):
    ...
    assert mask.ndim == 2
    mask = mask.transpose()
    strides = numpy.array([mask.shape[1], 1], int)
...
def get_pixel_data(img):
    ...
    #
    a = numpy.zeros(numpy.prod(dims), numpy.float64)
    ja = J.get_env().make_double_array(numpy.ascontiguousarray(a))
    strides = numpy.cumprod([1] + dims[:0:-1]).astype(int)[::-1]
    J.static_call("net/imglib2/util/ImgUtil", "copy",
...
def get_bit_data(img):
    ...
    #
    a = numpy.zeros(numpy.prod(dims), numpy.float64)
    ja = J.get_env().make_boolean_array(numpy.ascontiguousarray(a))
    strides = numpy.cumprod([1] + dims[:0:-1]).astype(int)[::-1]
    J.static_call("net/imglib2/util/ImgUtil", "copy",
